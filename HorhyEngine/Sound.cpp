...
bool Sound::Initialize(HWND hwnd, std::wstring fileName)
{
	bool result;
	...
	soundName = Engine::m_pPaths[PATH_TO_SOUNDS] + fileName;
	char *filePath = new char[255];
	sprintf(filePath, "%ls", soundName.c_str());

	// Load a wave audio file onto a secondary buffer.
	result = LoadWaveFile(filePath, &m_secondaryBuffer1, &m_secondary3DBuffer1);
	if (!result)
	{
		return false;
	}
